        ...
        <div class="row">
            <div class="col-lg-12">
                <x-breadcrumb :items="[
                    ...
                    ['title' => isset($region) ? __('areas/region.edit_region') : __('areas/region.create_region')]
                ]" />
            </div>
        </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-bottom py-20">
                        <h5 class="mb-0 fw-500">
                            {{ isset($region) ? __('areas/region.edit_region') : __('areas/region.create_region') }}
                        </h5>
                    </div>
                    <div class="card-body">
                        <!-- Alert Container -->
                        ...
                        @if ($errors->any())
                            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                <strong>{{ __('areas/region.validation_errors') }}</strong>
                                <ul class="mb-0 mt-2">
                        ...
                        <form id="regionForm" 
                              action="{{ isset($region) ? route('admin.area-settings.regions.update', $region->id) : route('admin.area-settings.regions.store') }}" 
                              method="POST">
                            @csrf
                            @if(isset($region))
                                @method('PUT')
                                    ...
                                    <div class="col-md-6 mb-25">
                                        <div class="form-group">
                                            <label for="translation_{{ $language->id }}_name" class="il-gray fs-14 fw-500 mb-10" @if($language->rtl) dir="rtl" style="text-align: right; display: block;" @endif>
                                                ...
                                                @else
                                                    {{ __('areas/region.name_english') }} ({{ $language->name }}) <span class="text-danger">*</span>
                                                @endif
                                            </label>
                                            <input type="text" 
                                                   ...
                                                   value="{{ isset($region) ? ($region->getTranslation('name', $language->code) ?? '') : old('translations.' . $language->id . '.name') }}"
                                                   placeholder="@if($language->code == 'ar')أدخل اسم المنطقة بالعربية@else{{ __('areas/region.enter_region_name_english') }}@endif"
                                ...
                                <div class="col-md-6 mb-25">
                                    <div class="form-group">
                                        <label for="city_id" class="il-gray fs-14 fw-500 mb-10">
                                            {{ __('areas/region.city') }} <span class="text-danger">*</span>
                                        </label>
                                        <div class="dm-select">
                                            <select class="form-control select2-city @error('city_id') is-invalid @enderror" 
                                                    id="city_id" 
                                                    name="city_id">
                                                <option value="">{{ __('areas/region.select_city') }}</option>
                                                @foreach($cities as $city)
                                                    <option value="{{ $city['id'] }}" 
                                                        {{ old('city_id', $region->city_id ?? $selectedCityId ?? '') == $city['id'] ? 'selected' : '' }}>
                                                        {{ $city['name'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="dm-switch-wrap d-flex align-items-center">
                                            <div class="form-check form-switch form-switch-primary form-switch-md">
                                                ...
                                                <input type="checkbox" 
                                                       ...
                                                       name="active" 
                                                       value="1"
                                                       {{ old('active', $region->active ?? 0) == 1 ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                        @error('active')
                            ...
                            <div class="d-flex justify-content-end gap-15 mt-30">
                                <a href="{{ route('admin.area-settings.regions.index') }}" 
                                   class="btn btn-light btn-default btn-squared fw-400 text-capitalize">
                                    <i class="uil uil-angle-left"></i> {{ __('areas/region.cancel') }}
                                </a>
                                <button type="submit" id="submitBtn" 
                                        class="btn btn-primary btn-default btn-squared text-capitalize"
                                        style="display: inline-flex; align-items: center; justify-content: center;">
                                    <i class="uil uil-check"></i> 
                                    <span>{{ isset($region) ? __('areas/region.update_region') : __('areas/region.add_region') }}</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('.select2-city').select2({
                placeholder: '{{ __('areas/region.select_city') }}',
                allowClear: true,
                width: '100%'
            });
