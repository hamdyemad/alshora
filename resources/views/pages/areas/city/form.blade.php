        ...
        <div class="row">
            <div class="col-lg-12">
                <x-breadcrumb :items="[
                    ...
                    ['title' => isset($city) ? __('areas/city.edit_city') : __('areas/city.create_city')]
                ]" />
            </div>
        </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-bottom py-20">
                        <h5 class="mb-0 fw-500">
                            {{ isset($city) ? __('areas/city.edit_city') : __('areas/city.create_city') }}
                        </h5>
                    </div>
                    <div class="card-body">
                        <!-- Alert Container -->
                        ...
                        @if ($errors->any())
                            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                <strong>{{ __('areas/city.validation_errors') }}</strong>
                                <ul class="mb-0 mt-2">
                        ...
                        <form id="cityForm" 
                              action="{{ isset($city) ? route('admin.area-settings.cities.update', $city->id) : route('admin.area-settings.cities.store') }}" 
                              method="POST">
                            @csrf
                            @if(isset($city))
                                @method('PUT')
                                    ...
                                    <div class="col-md-6 mb-25">
                                        <div class="form-group">
                                            <label for="translation_{{ $language->id }}_name" class="il-gray fs-14 fw-500 mb-10" @if($language->rtl) dir="rtl" style="text-align: right; display: block;" @endif>
                                                ...
                                                @else
                                                    {{ __('areas/city.name_english') }} ({{ $language->name }}) <span class="text-danger">*</span>
                                                @endif
                                            </label>
                                            <input type="text" 
                                                   ...
                                                   placeholder="@if($language->code == 'ar')أدخل اسم المدينة بالعربية@else{{ __('areas/city.enter_city_name_english') }}@endif"
                                ...
                                <div class="col-md-6 mb-25">
                                    <div class="form-group">
                                        <label for="country_id" class="il-gray fs-14 fw-500 mb-10">
                                            {{ __('areas/city.country') }} <span class="text-danger">*</span>
                                        </label>
                                        <div class="dm-select">
                                            <select class="form-control select2-country @error('country_id') is-invalid @enderror" 
                                                    id="country_id" 
                                                    name="country_id">
                                                <option value="">{{ __('areas/city.select_country') }}</option>
                                                @foreach($countries as $country)
                                                    <option value="{{ $country['id'] }}" 
                                                        {{ old('country_id', $city->country_id ?? $selectedCountryId ?? '') == $country['id'] ? 'selected' : '' }}>
                                                        {{ $country['name'] }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="dm-switch-wrap d-flex align-items-center">
                                            <div class="form-check form-switch form-switch-primary form-switch-md">
                                                ...
                                                <input type="checkbox" 
                                                       ...
                                                       name="active" 
                                                       value="1"
                                                       {{ old('active', $city->active ?? 1) == 1 ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                        @error('active')
                            ...
                            <div class="d-flex justify-content-end gap-15 mt-30">
                                <a href="{{ route('admin.area-settings.cities.index') }}" 
                                   class="btn btn-light btn-default btn-squared fw-400 text-capitalize">
                                    <i class="uil uil-angle-left"></i> {{ __('areas/city.cancel') }}
                                </a>
                                <button type="submit" id="submitBtn" 
                                        class="btn btn-primary btn-default btn-squared text-capitalize"
                                        style="display: inline-flex; align-items: center; justify-content: center;">
                                    <i class="uil uil-check"></i> 
                                    <span>{{ isset($city) ? __('areas/city.update_city') : __('areas/city.add_city') }}</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('.select2-country').select2({
                placeholder: '{{ __("areas/city.select_country") }}',
                allowClear: true,
                width: '100%'
            });
            ...
            cityForm.addEventListener('submit', function(e) {
                ...
                LoadingOverlay.animateProgressBar(30, 300).then(() => {
                    ...
                    // Debug: Log the active value and translations
                    console.log('Active value being sent:', formData.get('active'));
                    console.log('All form data:', Array.from(formData.entries()));
                    
                    // Log translations specifically
                    const translations = {};
                    for (let [key, value] of formData.entries()) {
                        if (key.startsWith('translations[')) {
                            console.log('Translation field:', key, '=', value);
                        }
                    }
