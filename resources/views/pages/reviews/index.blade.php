            ...
            <div class="card">
                <div class="card-header color-dark fw-500">
                    <div class="d-flex justify-content-between align-items-center w-100">
                        <div>
                            <h4>{{ trans('reviews.reviews') }} - {{ $lawyer->getTranslation('name', app()->getLocale()) }}</h4>
                            <div class="mt-2">
                                <span class="badge badge-info">{{ trans('reviews.average_rating') }}: <strong>{{ round($averageRating, 2) ?? 0 }}/5</strong></span>
                                <span class="badge badge-secondary ms-2">{{ trans('reviews.total_reviews') }}: <strong>{{ $totalReviews }}</strong></span>
                            </div>
                        </div>
                        <a href="{{ route('reviews.create', $lawyer->id) }}" class="btn btn-primary">
                            <i class="las la-plus"></i> {{ trans('reviews.add_review') }}
                        </a>
                    </div>
                </div>
                <div class="card-body">
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle text-center">
                                            <ul class="orderDatatable_actions mb-0 d-flex flex-wrap justify-content-center gap-1">
                                                <li>
                                                    <a href="{{ route('reviews.show', [$lawyer->id, $review->id]) }}" class="view" title="{{ trans('common.view') }}">
...
<x-delete-modal
    ...
    deleteRoute="{{ url('/') }}/{{ app()->getLocale() }}/reviews/{{ $lawyer->id }}"
